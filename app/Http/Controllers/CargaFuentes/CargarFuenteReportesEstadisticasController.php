            ...
            while (($data = fgetcsv($handle, 5000, ',')) !== FALSE) {
                ...
                if (isset($prestador->id) && isset($puntoAtencion->id) && isset($periodo->id)) {
                    ...
                    if (isset($funcionario->id)) {
                        ...
                        Log::info(', No tiene funcionario relacionado, ' . $prestadorMigracionId . ', ' . $puntoAtencionMigracionId . ', ' . $vigenciaArchivo . ', ' . $mesArchivo);
                    }
                } else {
                    if (!isset($prestador->id)) Log::info(', No tiene relacionado prestador, ' . $prestadorMigracionId . ', ' . $puntoAtencionMigracionId . ', ' . $vigenciaArchivo . ', ' . $mesArchivo);
                    if (!isset($puntoAtencion->id)) Log::info(', No tiene relacionado punto atencion, ' . $prestadorMigracionId . ', ' . $puntoAtencionMigracionId . ', ' . $vigenciaArchivo . ', ' . $mesArchivo);
                    if (!isset($periodo->id)) Log::info(', No tiene relacionado periodo, ' . $prestadorMigracionId . ', ' . $puntoAtencionMigracionId . ', ' . $vigenciaArchivo . ', ' . $mesArchivo);
                }
            }
